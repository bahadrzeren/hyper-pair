...
//System.out.println();
				...
				if ((nd.getNumOfLegsPassive() == 0)
					...
					if (nd.isHbArr(this.hbNdx)
							 ...
							 */
							&& (ChronoUnit.DAYS.between(fd.getBriefDayBeginning(this.hbNdx), nd.getDebriefDayEnding(this.hbNdx).minusSeconds(1)) < (toNdxExc - fromNdxInc + HeurosSystemParam.pairEnumIdleDayBuffer))) {
...
//System.out.println();
				...
				if ((pd.getNumOfLegsPassive() == 0)
					...
					if (pd.isHbDep(this.hbNdx)
						...
						if (hbArr) {
							if (ChronoUnit.DAYS.between(pd.getBriefDayBeginning(this.hbNdx), ld.getDebriefDayEnding(this.hbNdx).minusSeconds(1)) < (toNdxExc - fromNdxInc + HeurosSystemParam.pairEnumIdleDayBuffer)
									) {
							...
							if ((toNdxExc - fromNdxInc < HeurosSystemParam.pairEnumerationDept)
									&& ChronoUnit.DAYS.between(pd.getBriefDayBeginning(this.hbNdx), ld.getDebriefDayEnding(this.hbNdx).minusSeconds(1)) < (toNdxExc - fromNdxInc + HeurosSystemParam.pairEnumIdleDayBuffer)
									) {
								pairing[fromNdxInc] = pd;
								this.fwSearch(dutyStates,
												numOfProbablePairings,
